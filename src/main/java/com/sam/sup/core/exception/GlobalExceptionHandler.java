...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<@NonNull ErrorResult> handleBusiness(
      BusinessException exception, HttpServletRequest servletRequest) {
  ...
  public ResponseEntity<@NonNull ErrorResult> handleGeneric(
      Exception exception, HttpServletRequest servletRequest) {
  ...
  public ResponseEntity<ErrorResult> handleEnumParse(
      HttpMessageNotReadableException exception, HttpServletRequest servletRequest) {
    Throwable cause = exception.getCause();
    while (cause != null) {
      if (cause instanceof BusinessException be) {
        return ResultFactory.error(be.getErrorCode(), servletRequest.getServletPath());
      }
      cause = cause.getCause();
    }
    return ResultFactory.error(ErrorCode.INVALID_REQUEST, servletRequest.getServletPath());
  }
}
